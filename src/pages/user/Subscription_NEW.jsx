...
import { Calendar, MapPin, Clock, DollarSign, Star, AlertCircle, CheckCircle, Users } from "lucide-react";
...
const Subscription = () => {
  ...
  const calculateStopsBetween = () => {
    ...
    const stops = [
      ...
      ...selectedRoute.stops.map((s, idx) => ({ id: s._id, order: idx + 1 })),
      { id: `end_${selectedRoute._id}`, order: selectedRoute.stops.length + 1 },
    ];
    
    const pickupIndex = stops.findIndex(s => s.id === pickupStop._id);
    const dropIndex = stops.findIndex(s => s.id === dropStop._id);
    
    if (pickupIndex === -1 || dropIndex === -1 || pickupIndex >= dropIndex) return 0;
  ...
  const handlePurchase = async () => {
    ...
    if (starsBalance < pricing.totalCost) {
      setError(`Insufficient stars! You need ${pricing.totalCost - starsBalance} more stars.`);
      return;
    }
    ...
    try {
      ...
      alert(`Success! Subscription purchased for ${pricing.days} days (${response.data.ridesCount} rides)`);
  ...
  const getAvailableStops = () => {
    ...
    return [
      { _id: `start_${selectedRoute._id}`, name: selectedRoute.startPoint.name, type: 'start' },
      ...selectedRoute.stops.map(s => ({ _id: s._id, name: s.name, type: 'stop' })),
      { _id: `end_${selectedRoute._id}`, name: selectedRoute.endPoint.name, type: 'end' },
    ];
  };
  ...
  return (
    <Layout title="Purchase Subscription">
      <div className="max-w-6xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Purchase Subscription</h1>
          <p className="text-gray-600">Select a ride and customize your subscription plan</p>
          
          {/* Stars Balance */}
          <div className="mt-4 inline-flex items-center gap-2 bg-yellow-50 border border-yellow-200 px-4 py-2 rounded-lg">
            <Star className="w-5 h-5 text-yellow-500 fill-yellow-500" />
            <span className="font-semibold text-yellow-800">{starsBalance} Stars Available</span>
          </div>
        </div>
        ...
        <div className="mb-8 flex items-center justify-center gap-4">
          {[1, 2, 3, 4].map((num) => (
            <React.Fragment key={num}>
              ...
              {num < 4 && (
                <div className={`w-12 h-1 ${step > num ? 'bg-blue-600' : 'bg-gray-200'}`}></div>
              )}
            </React.Fragment>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
            <h2 className="text-xl font-semibold mb-4">Step 1: Select Date & Route</h2>
        ...
        {step === 2 && (
          <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
            <h2 className="text-xl font-semibold mb-4">Step 2: Select a Ride</h2>
            
            <div className="mb-4 p-4 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800">
                <strong>Route:</strong> {selectedRoute?.name}<br />
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">No rides available for this route on this date</p>
              </div>
            ) : (
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="text-lg font-semibold">{ride.scheduledStartTime}</span>
                        </div>
                        <div className="text-sm text-gray-600 space-y-1">
                          <p><strong>Driver:</strong> {ride.driver_id?.name || 'N/A'}</p>
                          <p><strong>Vehicle:</strong> {ride.vehicle_id?.model} ({ride.vehicle_id?.license_plate})</p>
                        </div>
                      </div>
        ...
        {step === 3 && (
          <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
            <h2 className="text-xl font-semibold mb-4">Step 3: Select Pickup/Drop & Plan</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const stop = getAvailableStops().find(s => s._id === e.target.value);
                    setPickupStop(stop);
                  }}
            ...
            {pricing.stopsCount > 0 && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h3 className="font-semibold text-green-800 mb-2">Pricing Summary</h3>
                <div className="space-y-1 text-sm text-green-700">
                  <p>Stops: {pricing.stopsCount} × 10 stars = {pricing.dailyCost} stars/day</p>
                  <p>Duration: {pricing.days} days (pay for {pricing.payableDays} days)</p>
                  {pricing.savings > 0 && (
                    <p className="text-green-600 font-semibold">Savings: {pricing.savings} stars!</p>
                  )}
        ...
        {step === 4 && (
          <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
            <h2 className="text-xl font-semibold mb-4">Step 4: Confirm Purchase</h2>
            
            <div className="space-y-4">
              <div className="border-b pb-3">
                <h3 className="font-semibold text-gray-700">Route Details</h3>
                <p className="text-gray-600">{selectedRoute?.name}</p>
                <p className="text-sm text-gray-500">{selectedRide?.scheduledStartTime}</p>
              </div>
              ...
              <div className="border-b pb-3">
                ...
                <p className="text-gray-600">{planType.charAt(0).toUpperCase() + planType.slice(1)} ({pricing.days} days)</p>
                <p className="text-sm text-gray-500">Starting: {new Date(selectedDate).toLocaleDateString()}</p>
              </div>

              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-semibold text-blue-900">Total Cost:</span>
                  <span className="text-2xl font-bold text-blue-900">{pricing.totalCost} ⭐</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-blue-700">Your Balance:</span>
                  <span className="font-semibold text-blue-700">{starsBalance} ⭐</span>
                </div>
                <div className="flex justify-between items-center text-sm mt-1">
                  <span className="text-blue-700">After Purchase:</span>
                  <span className="font-semibold text-blue-700">{starsBalance - pricing.totalCost} ⭐</span>
                </div>
              </div>
            </div>
